<!doctype html>
<html class="no-js" lang="">
<head>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
    <title>Srijan | Process</title>
    <meta name="description" content="">
    <meta name="viewport" content="width=device-width, initial-scale=1">

    <link rel="apple-touch-icon" href="apple-touch-icon.png">
    <!-- Place favicon.ico in the root directory -->

    <link rel="stylesheet" href="css/style.css">
    
    <script src="js/vendor/modernizr-2.8.3.min.js"></script>
</head>
<body class="inner-page">
    <div class="canvas">

    <div class="top-banner">
            <div class="top-band">
        <div class="row page-wrap">
          <div class="col-5">
            <a href="#" class="new-item">
              <span><i class="fa fa-video-camera" aria-hidden="true"></i>
               Developing Layouts in Backdrop CMS, The Drupal Fork </span> <i class="fa fa-long-arrow-right"></i>
             </a>
           </div>
           <div class="col-7">
            <nav class="secondary-nav nav">
              <ul class="nav-list">
                <li><a href="#nogo" accesskey="w">Webinars</a></li>
                <li><a href="#nogo">Blog</a></li>
                <li><a href="#nogo">About</a></li>
                <li><a href="#nogo">Careers</a></li>
                
                <li class="search"><form action="#">
                  <div class="search-bar">
                    <input type="text" class="keyword" placeholder="Search" />
                    <button><i class="fa fa-search"></i></button>
                  </div>
                </form>
                <div class="search-result">
                  <div class="item">
                    <a href="#">
                      <h3>Thought Article Title</h3>
                      Search Result Title with <span class="key-word">keyword</span> highlighted</a>
                    </div>
                    <div class="item">
                      <a href="#">
                        <h3>Page Name</h3>
                        Search Result Title with <span class="key-word">keyword</span> highlighted
                      </a>
                    </div>

                    <div class="item">
                      <a href="#">
                        <h3>Blog Post Title</h3>
                        Search Result Title with <span class="key-word">keyword</span> highlighted
                      </a>
                    </div>

                    <div class="item">
                      <a href="#">
                        <h3>Thought Article Title</h3>
                        Search Result Title with <span class="key-word">keyword</span> highlighted</a>
                      </div>

                      <a href="#" class="all-results">View All Results <i class="fa fa-long-arrow-right"></i></a>

                    </div>
                  </li>
                </ul>
              </nav>
            </div>
          </div>
        </div>
    <div class="row page-wrap header-top">
      <a class="col-2 logo" href="#nogo" title="Srijan">
        <img src="img/logo.png" alt="srijan logo">
        <span class="name">Srijan</span>
      </a>
      <div class="col-10">
        <nav class="primary-nav nav">
          <a href="#nav" class="mobile-nav">
            <span class="menu-icon">
              <i class="line"></i>
              <i class="line"></i>
              <i class="line"></i>
            </span>
          </a>
          <ul class="main-nav">
            <li><a href="#nogo">Drupal <i class="fa fa-angle-down"></i></a>
              <div class="fat-nav">
                <div class="row">
                  <div class="col-3">
                    <h2>Drupal</h2>
                    <ul class="menu">
                      <li><a href="#">
                        <i class="fa fa-caret-right"></i>
                        Drupal Solutions</a>
                      </li>
                      <li><a href="#">
                        <i class="fa fa-caret-right"></i>
                        Drupal Rehab</a>
                      </li>

                      <li><a href="#">
                        <i class="fa fa-caret-right"></i>
                        Drupal Staff Augmentation</a>
                      </li>

                      <li><a href="#">
                        <i class="fa fa-caret-right"></i>
                        Drupal Contributions</a>
                      </li>
                    </ul>
                  </div>
                  <div class="col-4-half">
                    <h2>Success Story</h2>
                    <article class="case-study-item">
                      <figure class="thumb">
                        <img src="img/tuilogo.png" alt="">
                      </figure>
                      <div class="cs-text">
                        <h3><a href="#">
                          Increased Leads on New Drupal Website for TUI India
                        </a></h3>
                        <p>
                          Drupal website development with upgraded UX and multiple functionalities and integrations improves..
                        </p>
                        <a href="#" class="more-link">Read More <i class="fa fa-chevron-right"></i></a>
                      </div>
                    </article>
                  </div>
                  <div class="col-4-half">
                    <h2>From the Blog</h2>
                    <article class="case-study-item">
                      <figure class="thumb">
                        <img src="img/d8blog.png" alt="">
                      </figure>
                      <div class="cs-text">
                        <h3><a href="#">
                          Technical Discovery for Drupal 8 Project
                        </a></h3>
                        <p>
                         Our case-study was aimed at the companies/developers who are starting with Drupal 8 projects for the first time..
                       </p>
                       <a href="#" class="more-link">Read More <i class="fa fa-chevron-right"></i></a>
                     </div>
                   </article>
                 </div>
               </div>
             </div>
           </li>
           <li><a href="solutions.html">Solutions <i class="fa fa-angle-down"></i></a>
            <div class="fat-nav">
              <div class="row">
                <div class="col-3">
                  <h2>Solutions</h2>
                  <ul class="menu">
                    <li><a href="#">
                      <i class="fa fa-caret-right"></i>
                      Product Engineering</a></li>
                      <li><a href="#">
                        <i class="fa fa-caret-right"></i>
                        Drupal Rehab</a></li>

                        <li><a href="#">
                          <i class="fa fa-caret-right"></i>
                          Sotfware Prototyping</a></li>

                          <li><a href="#">
                            <i class="fa fa-caret-right"></i>
                            Enterprise Solutions</a></li>

                            <li><a href="#">
                              <i class="fa fa-caret-right"></i>
                              UX + UI Design</a></li>

                            </ul>
                          </div>
                          <div class="col-4-half">
                            <h2>Success Story</h2>
                            <article class="case-study-item">
                              <figure class="thumb">
                                <img src="img/tuilogo.png" alt="">
                              </figure>
                              <div class="cs-text">
                                <h3><a href="#">
                                  Increased Leads on New Drupal Website for TUI India

                                </a></h3>
                                <p>
                                  Drupal website development with upgraded UX and multiple functionalities and integrations improves..
                                </p>
                                <a href="#" class="more-link">Read More <i class="fa fa-chevron-right"></i></a>
                              </div>

                            </article>

                          </div>
                          <div class="col-4-half">
                            <h2>From the Blog</h2>
                            <article class="case-study-item">
                              <figure class="thumb">
                                <img src="img/d8blog.png" alt="">
                              </figure>
                              <div class="cs-text">
                                <h3><a href="#">
                                  Technical Discovery for Drupal 8 Project

                                </a></h3>
                                <p>
                                 Our case-study was aimed at the companies/developers who are starting with Drupal 8 projects for the first time..
                               </p>
                               <a href="#" class="more-link">Read More <i class="fa fa-chevron-right"></i></a>
                             </div>

                           </article>
                         </div>
                       </div>
                     </div>
                   </li>
                   <li><a href="#nogo">Technologies <i class="fa fa-angle-down"></i></a>
                     <div class="fat-nav">
                      <div class="row">
                        <div class="col-3">
                          <h2>Technologies</h2>
                          <ul class="menu">
                            <li><a href="#">
                              <i class="fa fa-caret-right"></i>
                              Drupal</a></li>
                              <li><a href="#">
                                <i class="fa fa-caret-right"></i>
                                Front-end Development</a></li>

                                <li><a href="#">
                                  <i class="fa fa-caret-right"></i>
                                  JavaScript</a></li>

                                  <li><a href="#">
                                    <i class="fa fa-caret-right"></i>
                                    Ruby On Rails</a></li>

                                    <li><a href="#">
                                      <i class="fa fa-caret-right"></i>
                                      Angular &amp; MV*</a></li>


                                    </ul>
                                  </div>
                                  <div class="col-4-half">
                                    <h2>Success Story</h2>
                                    <article class="case-study-item">
                                      <figure class="thumb">
                                        <img src="img/tuilogo.png" alt="">
                                      </figure>
                                      <div class="cs-text">
                                        <h3><a href="#">
                                          Increased Leads on New Drupal Website for TUI India

                                        </a></h3>
                                        <p>
                                          Drupal website development with upgraded UX and multiple functionalities and integrations improves..
                                        </p>
                                        <a href="#" class="more-link">Read More <i class="fa fa-chevron-right"></i></a>
                                      </div>

                                    </article>

                                  </div>
                                  <div class="col-4-half">
                                    <h2>From the Blog</h2>
                                    <article class="case-study-item">
                                      <figure class="thumb">
                                        <img src="img/d8blog.png" alt="">
                                      </figure>
                                      <div class="cs-text">
                                        <h3><a href="#">
                                          Technical Discovery for Drupal 8 Project

                                        </a></h3>
                                        <p>
                                         Our case-study was aimed at the companies/developers who are starting with Drupal 8 projects for the first time..
                                       </p>
                                       <a href="#" class="more-link">Read More <i class="fa fa-chevron-right"></i></a>
                                     </div>

                                   </article>
                                 </div>
                               </div>
                             </div>
                           </li>
                           <li class="active"><a href="#nogo">Process <i class="fa fa-angle-down"></i></a>
                            <div class="fat-nav">

                            </div>
                          </li>
                          <li><a href="#nogo">Clients <i class="fa fa-angle-down"></i></a>
                            <div class="fat-nav">

                            </div>
                          </li>
                          <li><a href="#nogo">Contact <i class="fa fa-angle-down"></i></a>
                            <div class="fat-nav">

                            </div>
                          </li>
                        </ul>
                      </nav>
                    </div>
                  </div>
                </div>
    <main class="page process-page">
      <section class="process-section">
        <div class="ps-wrap row">
          <aside class="process-nav col-3">
            <ul class="process-side-nav">
              <li class="active">
                <a href="#">
                  <span class="icon-small">
                    <?php echo file_get_contents("icons/idea.svg"); ?>
                  </span>
                  <span class="title"><strong>1.</strong> Pre-Engagement</span>
                </a>
                <ul class="child-menu">
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Empowered Teams</span>
                    </a>
                  </li>
                  <li  class="current"><a href="process-weekly-burn-rate.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Weekly Burn Rate Model</span>
                    </a>
                  </li>
                  <li><a href="process-scrum-team.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Ideal Scrum Team</span>
                    </a>
                  </li>
                  <li><a href="process-agile-waterfall.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Agile Over Waterfall</span>
                    </a>
                  </li>
                  <li><a href="process-budget-quality.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Meeting budget without compromising quality</span>
                    </a>
                  </li>
                </ul>
              </li>
              <li>
                <a href="#">
                  <span class="icon-small">
                    <?php echo file_get_contents("icons/design.svg"); ?>
                  </span>
                  <span class="title"><strong>2.</strong> UX &amp; UI Design</span>
                </a>

                <ul class="child-menu">
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>UX Discovery</span>
                    </a>
                  </li>
                  <li><a href="process-weekly-burn-rate.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>User Research</span>
                    </a>
                  </li>
                  <li><a href="process-scrum-team.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>UX Design &amp; Wireframing</span>
                    </a>
                  </li>
                  <li><a href="process-agile-waterfall.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>UI Design</span>
                    </a>
                  </li>
                </ul>

              </li>
              <li>
                <a href="#">
                  <span class="icon-small">
                    <?php echo file_get_contents("icons/ux-design.svg"); ?>
                  </span>
                  <span class="title"><strong>3.</strong> Prototyping</span>
                </a>
                 <ul class="child-menu">
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Click-Through Prototypes</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>HTML, CSS &amp; JS Prototypes</span>
                    </a>
                  </li>
                </ul>
              </li>
              <li>
                <a href="#">
                  <span class="icon-small">
                    <?php echo file_get_contents("icons/windows.svg"); ?>
                  </span>
                  <span class="title"><strong>4.</strong> Tech Discovery</span>
                </a>
                <ul class="child-menu">
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Project Estimation</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Audit - Paid &amp; Free</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>POC - Spike</span>
                    </a>
                  </li>
                </ul>

              </li>
              <li>
                <a href="#">
                  <span class="icon-small">
                    <?php echo file_get_contents("icons/coding.svg"); ?>
                  </span>
                  <span class="title"><strong>5.</strong> Development Sprints</span>
                </a>

                <ul class="child-menu">
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Tracking Development on JIRA</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Backlog &amp; Sprint Planning</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Demos</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Retrospectives</span>
                    </a>
                  </li>
                </ul>


              </li>
              <li>
                <a href="#">
                  <span class="icon-small">
                    <?php echo file_get_contents("icons/server.svg"); ?>
                  </span>
                  <span class="title"><strong>6.</strong> DevOps</span>
                </a>

                <ul class="child-menu">
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Infrastructure</span>
                    </a>
                  </li>
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Platform.sh</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Aquia Cloud</span>
                    </a>
                  </li>

                </ul>
              </li>
              <li>
                <a href="#">
                  <span class="icon-small">
                    <?php echo file_get_contents("icons/qa.svg"); ?>
                  </span>
                  <span class="title"><strong>7.</strong> QA and Testing</span>
                </a>
                <ul class="child-menu">
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Realtime User Testing with Demo Script</span>
                    </a>
                  </li>
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>JMeter, New Relic, Blazemeter-Improving performance of a website</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Manual Testing</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Automated Testing</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Definition of Done</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Acceptance Criteria </span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Performance Testing - Backfire </span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Penetration Testing - Qwalus </span>
                    </a>
                  </li>

                </ul>

              </li>
              <li>
                <a href="#">
                  <span class="icon-small">
                    <?php echo file_get_contents("icons/communication.svg"); ?>
                  </span>
                  <span class="title"><strong>8.</strong> Communication</span>
                </a>
                 <ul class="child-menu">
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Single View Status Report</span>
                    </a>
                  </li>
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Retros and tools</span>
                    </a>
                  </li>
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Burndown Charts for Agile Reporting</span>
                    </a>
                  </li>
                </ul>
              </li>
              <li>
                <a href="#">
                  <span class="icon-small">
                    <?php echo file_get_contents("icons/employee.svg"); ?>
                  </span>
                  <span class="title"><strong>9.</strong> Support</span>
                </a>
                <ul class="child-menu">
                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Model</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Sprint Planning</span>
                    </a>
                  </li>

                  <li><a href="process.php">
                    <i class="fa fa-arrow-right"></i>
                    <span>Reporting</span>
                    </a>
                  </li>

                </ul>
              </li>
            </ul>

          </aside>
          <article class="ps-main col-9">
           
            <section class="article-subsection" id="burnRate">
              <h4>Pre-Engagement</h4>
              <h1>Agile Project Teams With Weekly Burn Rate Model</h1>
              <header class="sub-header">
                <?php echo file_get_contents("img/header-burnrate.svg"); ?>
              </header>
              <h2>The need for a dedicated team</h2>
              <p>Gerald Weinberg in his book "Quality Software Management: Systems Thinking" says that developers lose productive work-time switching between development tasks on multiple projects. The loss of efficiency can be over 50%, and with this suffers productivity and quality. Eventually, the project delivered is nearly always of poorer quality, and rolled-out just to get things out of the door, as the deadline looms large. The question really is—do you want a team which lives and breathes your project, or, do you want a team that is disengaged and disconnected with your success?</p>

              <h2>Typical Composition of SCRUM Team</h2>
              <p>A typical product development team has a Scrum Master, Developer(s), Front-end Engineer(s) and a Quality Assurance Engineer. Often our developers are multi-skilled people, and thus help make the team 'lean'. Scrum Teams are discussed in detail later.</p>

              <h2>Weekly Burn Rate Model</h2>
              <p>When Agile project teams are staffed largely in 100% capacity, and when they are focussed on the stories they are to complete in done-done state by the end of two weeks, it is natural for project estimates to be done based on a staffed team.</p>

              <figure class="feature-img full-width">
                <img src="img/burn-rate.png" alt="weekly burn rate">
              </figure>

            </section>


            <section class="related-content">
              <div class="row">
                <div class="col-4 rc-wrap blog">
                  <div class="rc-box">
                    <h3>Thought Article</h3>
                    <a href="#">
                    <figure class="thumbnail">
                      <img src="img/outsource.jpg" alt="">
                    </figure>
                    <h4>Why Outsourcing to India Will Prevail</h4>
                    <p>And how companies like Srijan will crack the code for quality, transparency and accountability.</p>
                    <span class="btn-span">Read Blog</span>
                    </a>
                  </div>
                </div>
                <div class="col-4 rc-wrap download">
                  <div class="rc-box">
                    <h3>Whitepaper</h3>
                    <a href="#">
                    <figure class="thumbnail">
                      <img src="img/wp-srijanteam.jpg" alt="">
                    </figure>
                    <h4>Srijan's Scrum Teams</h4>
                    <p>Learn how Srijan's Scrum Teams Seamlessly augmenting enterprise Drupal projects</p>
                    <span class="btn-span"><i class="fa fa-file-pdf-o"></i> Download</span>
                    </a>
                  </div>
                </div>

                <div class="col-4 rc-wrap case-study">
                  <div class="rc-box">
                    <h3>Case Study</h3>
                    <a href="#">
                    <figure class="thumbnail">
                      <img src="img/flight.jpg" alt="">
                    </figure>
                    <h4>OnCorps: Scalable Big Data Analytics Platform in Drupal </h4>
                    <p>complex analytics platform built on Drupal, using agile development methodology and dedicated teams</p>
                    <span class="btn-span">View Case Study</span>
                    </a>
                  </div>
                </div>
              </div>
            </section>
            <nav class="pagination-nav">
              <a href="process.php" class="previous"> <span class="s-icon"><i class="fa fa-chevron-circle-left"></i> </span> <strong>Previous:</strong> Empowered Teams</a>
              <a href="process-scrum-team.php" class="next"><strong>Next:</strong> Ideal Scrum Team <span class="s-icon"> <i class="fa fa-chevron-circle-right"></i></span></a>
            </nav>


            <nav class="pagination-nav fixed-page-nav">
              <a href="process.php" class="previous"> <span class="s-icon"><i class="fa fa-chevron-circle-left"></i> </span> <strong>Previous:</strong> Empowered Teams</a>
              <a href="process-scrum-team.php" class="next"><strong>Next:</strong> Ideal Scrum Team <span class="s-icon"> <i class="fa fa-chevron-circle-right"></i></span></a>
            </nav>


          </article>
        </div>
      </section>

    </main>



 <?php include 'contact-form.php';?>
    <?php include 'footer.php';?>

</div>
<div class="off-canvas">
  <ul class="menu-panel">
    <li><a href="#">Home</a></li>
    <li><a href="#">Drupal</a></li>
    <li><a href="#">Solutions</a></li>
    <li><a href="#">Technologies</a></li>
    <li><a href="#">Processes</a></li>
    <li><a href="#">Clients</a></li>
    <li><a href="#">Contact</a></li>
    <li><a href="#">Webinars</a></li>
    <li><a href="#">Blog</a></li>
    <li><a href="#">About</a></li>
    <li><a href="#">Careers</a></li>
  </ul>
</div>



<script src="https://ajax.googleapis.com/ajax/libs/jquery/1.11.3/jquery.min.js"></script>
<script>window.jQuery || document.write('<script src="js/vendor/jquery-1.11.3.min.js"><\/script>')</script>
<script src="js/plugins.js"></script>
<script src="js/main.js"></script>

</body>
</html>
